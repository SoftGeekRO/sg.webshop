...
class TagsField(CharField):
    ...
    def __init__(self, widget_settings=None, *args, **kwargs):
        # Widget settings
        self.widget_settings = {"delimiter": ","}
    ...
    def validate(self, value, model_instance):
        ...
        if "maxTags" in self.widget_settings:
            if len(tags) > self.widget_settings["maxTags"]:
                raise ValidationError(
                    "The number of tags specified is greater than maxTags."
                )
        ...
        if not no_tags:
            if (
                "duplicates" in self.widget_settings
                and self.widget_settings["duplicates"] is False
            ):
                for tag in tags:
                    if tags.count(tag) > 1:
            ...
            if (
                "enforceWhitelist" in self.widget_settings
                and self.widget_settings["enforceWhitelist"] is True
            ):
                for tag in tags:
